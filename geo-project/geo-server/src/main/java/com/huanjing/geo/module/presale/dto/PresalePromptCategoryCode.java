...
public enum PresalePromptCategoryCode {
    ...
    SCENARIO(
            ...
            "SCN",
            "中",
            "定位: 用户在与当前行业服务直接相关的消费触发点、使用时机或决策顾虑下寻求建议，最贴近真实决策路径。\n"
                    + "要点: 必须体现一个能解释“为什么此时需要该行业服务”的消费触发点或决策顾虑；预算、人群、时间、地点、使用状态等仅在符合行业和场景时自然选用，不得为了凑要素跨行业套用。以行业、品类或用户需求为主体，不出现任何具体品牌名；已填写目标用户时优先贴合该客群的决策语境，未填写时使用行业通用人群。"
    );
