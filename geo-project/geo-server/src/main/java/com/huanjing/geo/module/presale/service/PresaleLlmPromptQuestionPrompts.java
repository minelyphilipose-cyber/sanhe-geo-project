...
public final class PresaleLlmPromptQuestionPrompts {
    ...
    public static final String SCENARIO_GROUNDING_RULE = "生成前先在内部理解与当前行业服务直接相关的消费触发点和决策语境，再生成问题；不得将其他行业的生活场景机械套用过来。";

    public static final String SYSTEM_PROMPT = "你是售前 GEO（生成式引擎优化）报告的问题生成助手。请严格根据用户给定的品牌、行业、身份、地区、目标用户和客户诉求生成自然、简短、真实的中文用户问题。" + SCENARIO_GROUNDING_RULE;
    ...
    public static final String USER_PROMPT_TEMPLATE = """
            ...
            9. 问题应像真实用户口吻，简短、自然、可直接向 AI 提问，并以问号结尾。推荐型表达选择/推荐诉求；问题型必须同时包含真实顾虑和明确的决策追问（如哪些品牌、哪家服务商、怎么选更稳妥），使 AI 有理由自然推荐品牌或服务商，禁止只问“会不会有问题/靠不靠谱”；场景型必须有与当前行业服务直接相关的消费触发点或决策顾虑，预算、人群、使用时机等形式要素按实际场景自然选用，不强制出现。
