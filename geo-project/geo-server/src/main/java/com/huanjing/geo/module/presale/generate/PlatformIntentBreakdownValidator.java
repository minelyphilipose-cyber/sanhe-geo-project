...
public class PlatformIntentBreakdownValidator {
    ...
    private void validateRate(PlatformIntentCell cell) {
        int mentionCount = safeInt(cell.getMentionCount());
        int mentionRate = safeInt(cell.getMentionRate());
        ...
        if (mentionRate != expectedRate) {
            throw new BizException(500, "platform_intent_breakdown integrity violated: mention_rate formula mismatch, pair="
                    + pairKey(cell.getPlatformCode(), cell.getIntentCode()) + ", expected=" + expectedRate + ", actual=" + mentionRate);
        }
    }
