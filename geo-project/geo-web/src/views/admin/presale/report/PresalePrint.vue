...
import { getPresalePrintRenderDetail } from '@/api/presalePrint'
import {
  mergeSnapshot,
  type VersionRowMeta
...
function toVersionRowMeta(d: ReportDetailVO): VersionRowMeta {
  ...
  return {
    ...
    query_web_mode: v.queryWebMode,
    planned_query_count: v.plannedQueryCount,
    planned_web_query_count: v.plannedWebQueryCount,
    web_valid_query_count: v.webValidQueryCount,
    effective_sample_count: v.effectiveSampleCount,
    query_failed_count: v.queryFailedCount,
    analyze_failed_count: v.analyzeFailedCount,
    skipped_query_count: v.skippedQueryCount,
    degraded_excluded_sample_count: v.degradedExcludedSampleCount,
    main_web_failure_code: v.mainWebFailureCode,
...
function collectPageOverflowMetrics(): PageOverflowMetric[] {
  ...
  if (overflowPages.length > 0) {
    console.warn('[presale-print] page content overflows footer reserve', overflowPages)
  }
