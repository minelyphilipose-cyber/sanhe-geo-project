...
export const PROJECT_STAGE_MAP: Record<ProjectStage, {
  label: string
  color: string       // Element Plus tag type 或自定义色
  order: number
}> = {
  ...
  biweekly_feedback:    { label: '双周反馈',   color: '#3B82F6', order: 6  },
  monthly_report:       { label: '月报阶段',   color: '#6366F1', order: 7  },
  quarterly_report:     { label: '季报阶段',   color: '#8B5CF6', order: 8  },
  needs_renewal:        { label: '需续费',     color: '#F59E0B', order: 9  },
  high_risk:            { label: '高风险',     color: '#EF4444', order: 10 },
  dispute_handling:     { label: '争议处理中', color: '#DC2626', order: 11 },
  completed:            { label: '已结束',     color: '#6B7280', order: 12 },
}
...
export const REPORT_TYPE_MAP: Record<ReportType, { label: string; icon: string }> = {
  ...
  quarterly:         { label: '季度报表',     icon: 'TrendCharts' },
...
export const ROLE_MAP: Record<RoleType, { label: string; desc: string; isPartner: boolean }> = {
  ...
  manager:          { label: '管理者',     desc: '业务决策与审批',      isPartner: false },
  delivery_manager: { label: '交付负责人', desc: '项目交付与报表复核',  isPartner: false },
